...
export type CompoundCanonicalizer = (
	parts: readonly EntityId[],
	world: World,
) => readonly EntityId[]
export type CompoundEnumerator = (
	world: World,
	domain: CompoundDomain,
) => Iterable<readonly EntityId[]>
export type MemberComparator = (
	left: EntityId,
	right: EntityId,
	world: World,
) => number
...
export class CompoundRejectedError extends Error {
	constructor(
		...
		super(
			`Compound [${parts.join(', ')}] is not accepted by domain "${domain}"`,
		)
...
function defaultCompare(left: unknown, right: unknown): number {
	...
	throw new Error(
		'orderBy values require compareValues unless they are numbers or strings',
	)
}
...
export class CompoundDomain {
	...
	readonly #enumerate: CompoundEnumerator
	readonly #accept:
		| ((parts: readonly EntityId[], world: World) => boolean)
		| undefined
	readonly #detectCollisions: boolean
	readonly #collisionState = new WeakMap<
		World,
		{ revision: number; tags: Map<Tag, readonly EntityId[]> }
	>()
	...
	constructor(
		...
		if (!Number.isSafeInteger(this.arity) || this.arity < 1) {
			throw new Error(
				`Compound domain "${this.name}" requires a positive safe-integer arity`,
			)
		}
	}
	...
	canonicalParts(
		...
		if (parts.length !== this.arity) {
			throw new Error(
				`Compound domain "${this.name}" expects ${this.arity} parts, got ${parts.length}`,
			)
		}
		for (const part of parts) {
			if (!this.has(world, part)) {
				throw new Error(
					`Entity "${part}" is not a member of compound domain "${this.name}"`,
				)
			}
		}
		...
		if (canonical.length !== this.arity) {
			throw new Error(
				`Compound domain "${this.name}" canonicalizer changed arity`,
			)
		}
		for (const part of canonical) {
			if (!this.has(world, part)) {
				throw new Error(
					`Compound domain "${this.name}" canonicalizer returned non-member "${part}"`,
				)
			}
		}
		if (
			...
			throw new Error(
				`Compound domain "${this.name}" does not allow repeated members`,
			)
		}
	...
	normalizeTag(world: World, value: Tag): Compound {
		if (!this.parseTag) {
			throw new Error(
				`Compound domain "${this.name}" has no parser for existing external tags`,
			)
		}
...
function* cartesianPower(
	values: readonly EntityId[],
	arity: number,
): IterableIterator<readonly EntityId[]> {
	if (arity === 0) {
		yield []
		return
	}
...
function symmetricEnumerator(compare: MemberComparator): CompoundEnumerator {
	return function* (world, domain) {
		const members = [...domain.memberIds(world)].sort((a, b) =>
			compare(a, b, world),
		)
...
const orderedEnumerator: CompoundEnumerator = function* (world, domain) {
	...
	for (const tuple of cartesianPower(members, domain.arity)) {
		if (domain.repeats === 'forbid' && new Set(tuple).size !== tuple.length)
			continue
		yield tuple
	}
}
...
export const compound = {
	symmetric<T = never>(options: SymmetricDomainOptions<T>): CompoundDomain {
		const compare: MemberComparator =
			options.compareMembers ??
			((left, right, world) => {
				...
				const compareValues =
					...
					(defaultCompare as (left: T, right: T) => number)
				return (
