...
function hierarchy(options: HierarchyRuleOptions): Rule {
	...
	const name =
		options.name ??
		`hierarchy:${setName(options.members)}:${relationName(options.parents)}`
	return customRule(
		name,
		({ world, emit }) => {
			for (const entity of setEntities(world, options.members)) {
				for (const parent of relationTargets(
					options.parents,
					entity.id,
					world,
				)) {
					if (
						options.parentScope &&
						!setHas(world, options.parentScope, parent)
					) {
						throw new Error(
							`Parent "${parent}" of "${entity.id}" is outside required scope "${setName(options.parentScope)}"`,
						)
					}
...
export class ProductTagCollisionError extends Error {
	constructor(
		...
		super(
			`Product rule "${ruleName}" maps both [${first.join(', ')}] and [${second.join(', ')}] to "${tag}"`,
		)
		this.name = 'ProductTagCollisionError'
	}
}

function* productTuples(
	world: World,
	axes: readonly ProductAxis[],
): IterableIterator<readonly EntityId[]> {
	const values = axes.map((axis) =>
		[...setEntities(world, axis.members)].map((entity) => entity.id),
	)
...
function product(options: ProductRuleOptions): Rule {
	if (options.axes.length === 0)
		throw new Error('Product rule requires at least one axis')
	const name =
		options.name ??
		`product:${options.axes.map((axis) => axis.name ?? setName(axis.members)).join('×')}`
	return customRule(
		name,
		({ world, emit }) => {
			...
			const materialize = (parts: readonly EntityId[]): Tag => {
				...
				if (options.detectTagCollisions ?? true) {
					...
					if (previous && tupleKey(previous) !== tupleKey(parts)) {
						throw new ProductTagCollisionError(
							name,
							result,
							previous,
							parts,
			...
			for (const parts of productTuples(world, options.axes)) {
				...
				for (let index = 0; index < options.axes.length; index++) {
					...
					for (const parent of relationTargets(
						...
						if (
							axis.parentScope &&
							!setHas(world, axis.parentScope, parent)
						) {
							throw new Error(
						...
						const lifted = [...parts]
						lifted[index] = parent
						if (options.accept && !options.accept(lifted, world))
							continue
...
function normalizeTags(
	value: Tag | Iterable<Tag> | null | undefined,
): readonly Tag[] {
			...
			for (const current of options.domain.compounds(world)) {
				if (options.decompose) {
					...
					for (let index = 0; index < current.parts.length; index++) {
						...
						for (const parent of normalizeTags(
							options.decompose(member, world, context),
						)) {
							if (emitted.has(parent) || parent === current.tag)
								continue
							emitted.add(parent)
				...
				if (options.derive) {
					for (const parent of new Set(
						options.derive(current, world),
					)) {
						if (parent !== current.tag)
							emit(current.tag, parent, {
								reason: 'compound-derived',
							})
					}
				}
				...
				if (options.inherit) {
					for (let index = 0; index < current.parts.length; index++) {
						...
						for (const parent of resolveMemberParents(
							options.inherit,
							member,
							world,
							context,
							...
							try {
								parentCompound = options.domain.normalize(
									world,
									lifted,
								)
							} catch (error) {
								// A domain compatibility predicate may intentionally make a lifted form invalid.
								if (error instanceof CompoundRejectedError)
									continue
								throw error
							}
