...
export class Query {
  ...
  *entities(world: World): IterableIterator<Entity> {
    for (const entity of world.entities()) if (this.test(entity, world)) yield entity;
  }
  ...
  and(...sets: readonly EntitySet[]): Query {
    ...
    return new Query(
      ...
      (entity, world) => this.test(entity, world) && sets.every((set) => test(set, entity, world)),
    );
  }
  ...
  or(...sets: readonly EntitySet[]): Query {
    ...
    return new Query(
      suffix ? `${this.name}|${suffix}` : this.name,
      (entity, world) => this.test(entity, world) || sets.some((set) => test(set, entity, world)),
    );
  }
  ...
  where(predicate: EntityPredicate, name = 'predicate'): Query {
    return new Query(
      `${this.name}&${name}`,
      (entity, world) => this.test(entity, world) && predicate(entity, world),
    );
  }
...
export function query(...sets: readonly EntitySet[]): Query {
  ...
  return new Query(name, (entity, currentWorld) => sets.every((set) => test(set, entity, currentWorld)));
}
