...
export class ImplicationGraph {
  ...
  children(parent: Tag): readonly Tag[] {
    const out: Tag[] = [];
    for (const [child, parents] of this.#parents) if (parents.has(parent)) out.push(child);
    return out;
  }
  ...
  add(child: Tag, parent: Tag, provenance: Provenance = { rule: 'manual' }): boolean {
    if (!child || !parent) throw new Error('Implication tags must not be empty');
  ...
  addAll(edges: Iterable<{ child: Tag; parent: Tag; provenance?: Provenance }>): number {
    let added = 0;
    for (const edge of edges) {
      if (this.add(edge.child, edge.parent, edge.provenance ?? { rule: 'manual' })) added++;
    }
    return added;
  }
  ...
  path(from: Tag, to: Tag): readonly Tag[] | undefined {
    ...
    const queue: Array<{ tag: Tag; path: Tag[] }> = [{ tag: from, path: [from] }];
  ...
  why(child: Tag, parent: Tag): Explanation | undefined {
    ...
    for (let index = 0; index + 1 < path.length; index++) {
      ...
      if (!edge) throw new Error('ImplicationGraph internal error: missing proof edge');
      edges.push(edge);
    }
    return { path, edges };
  }
  ...
  direct(): readonly ImplicationEdge[] {
    ...
    for (const parents of this.#parents.values()) {
      for (const edge of parents.values()) {
        out.push({ child: edge.child, parent: edge.parent, provenance: [...edge.provenance] });
      }
    }
    return out.sort((a, b) => a.child.localeCompare(b.child) || a.parent.localeCompare(b.parent));
  }
