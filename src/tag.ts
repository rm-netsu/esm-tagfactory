...
export const tag = {
  ...
  from(component: ReadableComponent<Tag>, fallback?: TagProjection): TagProjection {
    return (entity, world) => {
      const value = world.get(entity, component);
      if (value !== undefined) return value;
      if (fallback) return fallback(entity, world);
      throw new Error(`Entity "${entity}" has no "${component.name}" value required for tag materialization`);
    };
  },

  map(base: TagProjection, mapper: (value: Tag, entity: EntityId, world: World) => Tag): TagProjection {
  ...
  when(
    ...
    return (entity, world) => {
      const matches = typeof condition === 'function' && !('token' in condition)
  ...
  join(separator: string, part: TagProjection = id): CompoundRenderer {
    return (parts, world) => parts.map((entity) => part(entity, world)).join(separator);
  },
} as const;
