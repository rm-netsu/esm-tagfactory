...
export class Relation {
	...
	or(...others: readonly RelationLike[]): Relation {
		const base = this
		return new Relation(
			`${this.name}|${others.map(relationName).join('|')}`,
			function* (source: EntityId, world: World) {
	...
	inverse(name = `${this.name}^-1`): Relation {
		return new Relation(name, (target, world) => {
			...
			for (const entity of world.entities()) {
				if (this.targets(entity.id, world).includes(target))
					out.push(entity.id)
			}
			return out
		})
...
export function relation(
	sourceOrName: LinksComponent | string,
	...
	if (isLinksComponent(sourceOrName)) {
		...
		return new Relation(
			source.name,
			(entity, world) => world.get(entity, source) ?? [],
		)
	}
	if (!resolver)
		throw new Error(
			`Computed relation "${sourceOrName}" requires a resolver`,
		)
...
export function relationTargets(
	...
	return source instanceof Relation
		? source.targets(entity, world)
		: (world.get(entity, source) ?? [])
}
