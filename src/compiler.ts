	...
	constructor(
		...
		super(
			`World did not stabilize after ${maxPasses} compilation passes` +
				(changingRules.length
					? `; rules still mutating it: ${changingRules.join(', ')}`
					: ''),
		)
...
export class Compiler {
	...
	run(world: World, ...inputs: readonly RuleInput[]): Compilation {
		const rules = [...flattenRules(inputs)].sort(
		...
		for (let pass = 1; pass <= this.maxPasses; pass++) {
			...
			for (const currentRule of rules) {
				...
				const context: RuleContext = {
					...
					pass,
					emit,
					edge: ({ child, parent, reason, source }) =>
						emit(child, parent, {
				...
				if (world.revision !== ruleWorldBefore)
					changingRules.push(currentRule.name)
			}
