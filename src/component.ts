...
function metadata<T>(kind: ReadableComponent<T>['kind'], options: ComponentOptions<T>) {
  let name = '';
  return {
    token: Symbol(kind),
    kind,
    get name() { return name || '<unbound>'; },
    equals(left: T, right: T) {
      return options.equals?.(left, right) ?? Object.is(left, right);
    },
    [BIND_NAME](next: string) {
      if (name && name !== next) {
        throw new Error(`Component is already bound as "${name}" and cannot also be named "${next}"`);
      }
      name = next;
    },
  } as const;
...
export function field<T>(options: ComponentOptions<T> = {}): FieldComponent<T> {
  ...
  const fn = ((value: T): SetFact<T> => ({ kind: SET, component, value })) as FieldComponent<T>;
  component = decorateCallable<T, typeof fn>(fn, metadata('field', options)) as FieldComponent<T>;
  return component;
}
...
export function links(): LinksComponent {
  ...
  component = decorateCallable<readonly EntityId[], typeof fn>(fn, metadata('links', {
  ...
  component.replace = (...targets) => ({ kind: SET, component, value: [...targets] });
  return component;
}
...
export function isSetFact(value: Trait): value is SetFact<any> {
  return typeof value === 'object' && value !== null && 'kind' in value && value.kind === SET;
}

export function isAppendLinksFact(value: Trait): value is AppendLinksFact {
  return typeof value === 'object' && value !== null && 'kind' in value && value.kind === APPEND;
}
