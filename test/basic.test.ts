...
describe('esm-tagfactory core API', () => {
	test('original Hydrus scenarios compose cleanly', () => {
		...
		expect(graph.has('species:dragon', 'species:mythological_scalie')).toBe(
			true,
		)
	...
	test('product is N-dimensional', () => {
		...
		const graph = compile(
			w,
			rule.product({
				axes: [{ members: C.A }, { members: C.B }, { members: C.C }],
				render: tag.join('/'),
			}),
		)
	...
	test('custom rules participate in fixed-point world derivation', () => {
		...
		const materialize = rule.custom(
			'materialize',
			({ world: currentWorld, emit }) => {
				for (const entity of currentWorld.entities()) {
					if (entity.has(C.Derived))
						emit(`derived:${entity.id}`, entity.id)
				}
			},
		)
	...
	test('compound and product tag collisions fail loudly', () => {
		...
		const domain = compound.symmetric({
			members: C.X,
			render: () => 'same',
		})
		expect(() => [...domain.compounds(w)]).toThrow(
			CompoundTagCollisionError,
		)
		...
		try {
			...
			expect((error as RuleExecutionError).cause).toBeInstanceOf(
				ProductTagCollisionError,
			)
		}
	})
	...
	test('graph explains provenance, reduces transitives and rejects cycles', () => {
		...
		const w = world({
			a: [C.Node, C.Parent('b', 'c')],
			b: [C.Node, C.Parent('c')],
			c: [C.Node],
		})
		const graph = compile(
			w,
			rule.hierarchy({ members: C.Node, parents: C.Parent }),
		)
